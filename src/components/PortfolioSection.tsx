...
const portfolioItems = [
  {
    ...
    description: "Ultra-realistic — Emotional — Atmospheric — Makes people pause.",
  },
  {
    id: 2,
  ...
  ({
    ...
    return (
      ...
      >
        ...
        <div
          className="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden lg:block"
        />

        {/* Content - hidden on mobile for raw look */}
        <div
          className="absolute inset-0 flex-col justify-end p-6 hidden lg:flex opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300"
        >
          <span className="text-primary text-xs tracking-widest uppercase mb-2 font-cinzel">
            {item.category}
          </span>
          <h3 className="text-xl font-bold text-foreground mb-2 font-playfair">{item.title}</h3>
          <p className="text-muted-foreground text-sm mb-4">{item.description}</p>
          <div className="flex gap-3">
            <button
              className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center hover:scale-110 transition-transform duration-200"
            >
              <Play size={16} />
            </button>
            <button
              className="w-10 h-10 rounded-full border border-foreground text-foreground flex items-center justify-center hover:bg-foreground hover:text-background transition-colors duration-200"
            >
        ...
        <div
          className="absolute top-4 right-4 w-0 h-0 border-t-[40px] border-t-primary border-l-[40px] border-l-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden lg:block"
        />
      </motion.div>
    );
  }
      ...
      >
        ...
        >
          ...
          <div className="p-6 md:p-8">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-4 font-playfair">{item.title}</h3>
...
const PortfolioSection = memo(() => {
  ...
  return (
    <section id="portfolio" className="py-32 bg-background-secondary relative" ref={ref}>
      <div className="container mx-auto px-4 md:px-8">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-light font-playfair">
            FEATURED <span className="font-bold text-gradient-gold">PORTFOLIO</span>
          </h2>
        </motion.div>
